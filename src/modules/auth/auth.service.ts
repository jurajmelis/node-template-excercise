...
export class AuthService {
  ...
  public async login(data: LoginUserDto): Promise<AccessToken> {
    ...
    const token = sign(
      ...
      config.JWT_SECRET,
      { expiresIn: config.JWT_EXPIRES_AT * 3600 },
    );
  ...
  public getUserId(authorization: string): string {
    const token = authorization?.split(" ")[0] === "Bearer" ? authorization.split(" ")[1] : undefined;
    if (!token) {
      throw new UnprocessableEntityError("Unauthorized user: missing token");
    }
    const { id } = verify(token, config.JWT_SECRET) as { [id: string]: string };
    return id;
  }
}
